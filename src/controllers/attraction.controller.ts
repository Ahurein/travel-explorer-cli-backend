...
import { getAttractionByCityService, getAttractionByCountryService, getAttractionsByContinentService, getAttractionsNearYouService, getAttractionsStatsService, getContinentThingsToDoService, getCountryThingsToDoService } from "../services/attraction.service";
...
export const attractionsStats = async (req: Request, res: Response) => {
  try {
    const continentStats = await getAttractionsStatsService();
    return res.status(200).json(continentStats);
  } catch (error) {
    return res.status(400).json({ "message": "Encountered and error" });
  }
}
